...
- **자연어 Q&A** — "로그인 어디서 처리해?" → `file:line` 인용해서 답변. 근거 없으면 "못 찾음"(환각 방지). 심층 모드는 Critic 에이전트가 답변을 검토·재작성.
...
3. **QA 분석 실행** — 심각도별 발견사항. 상단 스탯 타일(High/Medium/Low/신규/해결됨) 클릭 시 해당 항목만 필터
4. **발견사항 클릭 → 상세 + 코딩 에이전트용 수정 프롬프트 복사**
5. **대화** — 우측 드로어에서 코드 질문 (🔬 심층 토글 시 Critic 검토 루프)

### 고침 확인 워크플로

코드 고침 → push → **재인덱싱**(URL로 가져오기) → **QA 재실행** →
상단 **✅ 해결됨** 타일에 고친 항목, **🆕 신규** 태그에 새로 생긴 것. (온도 0이라 안 바뀐 코드는 결과 동일)
